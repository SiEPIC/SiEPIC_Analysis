'''
SiEPIC openEBL Measurement Data Viewer
by Lukas Chrostowski, 2025
written with the help of ChatGPT 4.o and Claude

This script provides a GUI for viewing and analyzing measurement data from 
openEBL fabrication runs. It matches measurement files to layout devices using
GDS (x,y) coordinates extracted from CSV files

Features:
- Coordinate-based matching between measurement data and layout labels
- Caches CSV coordinate database to disk for fast subsequent loads
- Displays layout images with device highlighting
- Plots spectrum data from .mat files
- Exports SPICE netlists for selected devices
- Waveguide loss analysis (cutback method)

Setup:
    1. Put the layout file (e.g., D2025_10_11_EBeam.oas) in the same folder as viewer.py
    2. Put all measurement data (.csv and .mat files) in subfolders within the same directory
       The folder structure should look like:
           viewer.py
           D2025_10_11_EBeam.oas
           1550/TE/25C/.../device_name/timestamp.csv
           1550/TE/25C/.../device_name/timestamp.mat
           ...
...
Requirements:
    pip install PyQt5 matplotlib numpy scipy klayout SiEPIC siepic_ebeam_pdk
...
# Database file name for CSV coordinate cache
CSV_DATABASE_FILE = "csv_coordinates_db.json"


def build_csv_database(data_folder, force_rebuild=False):
    """
    Scans all CSV files in data_folder and subfolders, extracts device coordinates
    from line 7 (# Device coordinates (gds): x, y), and saves to a JSON database.
    Also stores the corresponding .mat file path for spectrum loading.
    
    Args:
        data_folder (str): Root folder to scan for CSV files.
        force_rebuild (bool): If True, rebuild the database even if it exists.
    
    Returns:
        dict: Database mapping CSV file paths to their (x, y) coordinates and .mat file paths.
    """
    db_path = os.path.join(data_folder, CSV_DATABASE_FILE)
    
    # Check if database exists and is up-to-date
    if not force_rebuild and os.path.exists(db_path):
        print(f"Loading existing CSV database from: {db_path}")
        with open(db_path, 'r') as f:
            db = json.load(f)
        print(f"  Loaded {len(db)} entries from database.")
        return db
    
    print(f"Building CSV coordinate database from: {data_folder}")
    db = {}
    csv_count = 0
    matched_count = 0
    ...
    for root, dirs, files in os.walk(data_folder):
        for file in files:
            if file.endswith('.csv'):
                ...
                coords = extract_coordinates_from_csv(csv_path)
                if coords:
                    # Find corresponding .mat file (same name, different extension)
                    mat_file = file.replace('.csv', '.mat')
                    mat_path = os.path.join(root, mat_file)
                    if not os.path.exists(mat_path):
                        mat_path = csv_path  # Fallback to CSV if no .mat exists
                    
                    # Store relative path for portability
                    rel_path = os.path.relpath(csv_path, data_folder)
                    db[rel_path] = {
                        'x': coords[0],
                        'y': coords[1],
                        'full_path': mat_path,  # Use .mat file for spectrum loading
                        'csv_path': csv_path,
    ...
    print(f"  Scanned {csv_count} CSV files, extracted coordinates from {matched_count} files.")
...
def extract_coordinates_from_csv(csv_path):
    """
    Reads a CSV file and extracts device GDS coordinates from line 7.
    Expected format: # Device coordinates (gds):     x, y
    
    Args:
        csv_path (str): Path to the CSV file.
    
    Returns:
        tuple: (x, y) coordinates as integers, or None if not found.
    """
    try:
        with open(csv_path, 'r') as f:
            lines = f.readlines()
            if len(lines) >= 7:
                line7 = lines[6]  # 0-indexed, so line 7 is index 6
                # Match pattern: # Device coordinates (gds): x, y
                match = re.search(r'#\s*Device coordinates \(gds\):\s*(-?\d+),\s*(-?\d+)', line7)
                if match:
                    x = int(match.group(1))
                    y = int(match.group(2))
                    return (x, y)
...
def extract_layout_label_coordinates(labels):
    """
    Extracts x, y coordinates from opt_in labels.
    
    Args:
        labels: Labels returned by find_automated_measurement_labels.
    
    Returns:
        dict: Mapping of (x, y) tuples to label info.
    """
    coord_to_label = {}
    for label in labels[1]:
        if 'opt_in' in label and 'x' in label and 'y' in label:
            x = label['x']
            y = label['y']
            coord_to_label[(x, y)] = label
    print(f"Extracted {len(coord_to_label)} label coordinates from layout.")
    return coord_to_label


def match_csv_to_labels_by_coordinates(csv_db, coord_to_label, tolerance=5):
    """
    Matches CSV files to layout labels using x, y coordinates.
    
    Args:
        csv_db (dict): CSV database with coordinates.
        coord_to_label (dict): Layout label coordinates.
        tolerance (int): Maximum distance (in microns) for coordinate matching.
    
    Returns:
        dict: Matches mapping device names to [csv_path, label_info].
    """
    matches = {}
    matched_count = 0
    unmatched_csv = []
    
    for rel_path, csv_info in csv_db.items():
        csv_x = csv_info['x']
        csv_y = csv_info['y']
        csv_path = csv_info['full_path']
        
        # Find matching label by coordinates
        best_match = None
        best_distance = float('inf')
        
        for (label_x, label_y), label_info in coord_to_label.items():
            distance = abs(csv_x - label_x) + abs(csv_y - label_y)
            if distance < best_distance:
                best_distance = distance
                best_match = label_info
        
        if best_match and best_distance <= tolerance:
            device_type = best_match.get('type', '')
            device_id = best_match.get('deviceID', '')
            params = "_".join(best_match.get('params', []))
            if params == 'comment':
                params = ''
            dev_name = f"{device_type}_{device_id}_{params}".strip('_')
            
            # Handle multiple measurements of same device
            if dev_name in matches:
                # Append to existing entry or create unique name
                base_name = dev_name
                counter = 2
                while dev_name in matches:
                    dev_name = f"{base_name}_{counter}"
                    counter += 1
            
            matches[dev_name] = [csv_path, best_match]
            matched_count += 1
        else:
            unmatched_csv.append((rel_path, csv_x, csv_y))
    
    print(f"\nCoordinate-based matching results:")
    print(f"  Matched: {matched_count} / {len(csv_db)} CSV files ({100*matched_count/len(csv_db):.1f}%)")
    print(f"  Unmatched: {len(unmatched_csv)} CSV files")
    
    if len(unmatched_csv) > 0 and len(unmatched_csv) <= 20:
        print("  Unmatched CSV files:")
        for rel_path, x, y in unmatched_csv:
            print(f"    - {rel_path} at ({x}, {y})")
    
    return matches, matched_count, len(csv_db)
    ...
    def update_tabs(self):
        ...
        for selected_key in selected_items:
            if selected_key in self.matches:
                data_file_path = self.matches[selected_key][0]  # Get the first associated file
                self.plot_data(data_file_path, selected_key, len(selected_items)>1)
                ...
                print(f" - Opt-in: {opt_in_selection_text}, {data_file_path}")
                try:
...
#                    print(text_subckt, text_main)
    ...
    def plot_data(self, file_path, title, multi=False):
        """
        Reads and plots the spectrum data from a .mat or .csv file.
        """
        if file_path.endswith('.mat'):
            self.plot_mat_data(file_path, title, multi)
        elif file_path.endswith('.csv'):
            self.plot_csv_data(file_path, title, multi)
    ...
    def plot_csv_data(self, csv_file_path, title, multi=False):
        """
        Reads and plots the spectrum data from a .csv file.
        """
        try:
            # Read CSV file, skipping header lines that start with #
            data = np.genfromtxt(csv_file_path, delimiter=',', comments='#')
            if data.ndim == 1:
                print(f"Warning: CSV file has only one column: {csv_file_path}")
                return
            ...
            for i in range(1, data.shape[1]):
                ...
                if np.max(spectrum_data) > CONST_NoiseFloor:
                    if multi:
                        self.ax.plot(wavelengths, spectrum_data, label=f"{title}:{i}")
                    else:
                        self.ax.plot(wavelengths, spectrum_data, label=f"channel:{i}")
...
def load_layout_and_extract_labels(data_folder):
    """
    Loads the layout file and extracts opt_in labels using SiEPIC.
    
    Returns:
        tuple: (layout, labels)
    """
    layout_path = os.path.abspath(os.path.join(data_folder, 'D2025_10_11_EBeam.oas'))
...
if __name__ == "__main__":
    ...
    # Load layout and extract labels
    layout, labels = load_layout_and_extract_labels(data_folder)
    
    # Build or load CSV coordinate database
    csv_db = build_csv_database(data_folder, force_rebuild=False)
    
    # Extract coordinates from layout labels
    coord_to_label = extract_layout_label_coordinates(labels)
    
    # Match CSV files to labels using coordinates
    matches, matched_count, total_csv = match_csv_to_labels_by_coordinates(csv_db, coord_to_label, tolerance=5)
    
    # Check match rate
    match_rate = matched_count / total_csv if total_csv > 0 else 0
    print(f"\nMatch rate: {match_rate:.1%}")
    
    if match_rate < 0.80:
        print("\nWARNING: Match rate is below 80%!")
        print("Attempting to debug coordinate matching...")
        
        # Print some sample coordinates from both sources
        print("\nSample CSV coordinates:")
        for i, (rel_path, info) in enumerate(list(csv_db.items())[:5]):
            print(f"  {info['folder']}: ({info['x']}, {info['y']})")
        
        print("\nSample layout label coordinates:")
        for i, ((x, y), label) in enumerate(list(coord_to_label.items())[:5]):
            print(f"  {label.get('deviceID', 'unknown')}: ({x}, {y})")
        
        # Try with larger tolerance
        print("\nRetrying with tolerance=10...")
        matches, matched_count, total_csv = match_csv_to_labels_by_coordinates(csv_db, coord_to_label, tolerance=10)
        match_rate = matched_count / total_csv if total_csv > 0 else 0
        print(f"New match rate: {match_rate:.1%}")
